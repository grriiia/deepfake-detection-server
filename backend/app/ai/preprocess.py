...
def extract_frames(filename, num_frames, model, image_size=(380, 380)):
    ...
    for cnt_frame in frame_idxs:
        cap_org.set(cv2.CAP_PROP_POS_FRAMES, cnt_frame)
        ret_org, frame_org = cap_org.read()
        if not ret_org or frame_org is None:
            tqdm.write(f"Frame read {cnt_frame} Error! : {os.path.basename(filename)}")
            continue
        ...
        try:
            if len(faces) == 0:
                tqdm.write(f"No faces in {cnt_frame}:{os.path.basename(filename)}")
                continue
            ...
            for face_idx in range(len(faces)):
                bbox_data = faces[face_idx].get("bbox", [])
                if len(bbox_data) != 4:
                    continue
            ...
            croppedfaces_temp = [
                f for face_idx, f in enumerate(croppedfaces_temp)
                if size_list[face_idx] >= max_size / 2
            ]
            idx_list_temp = [
                f for face_idx, f in enumerate(idx_list_temp)
                if size_list[face_idx] >= max_size / 2
            ]
